classdef (StrictDefaults)DigitalRead < matlabshared.svd.DigitalRead ...
            ...
            if ~coder.target('Rtw') && ~coder.target('Sfun')
                if ~isempty(obj.Hw)
                    if ~isValidDigitalPin(obj.Hw,value)
                        error(message('svd:svd:PinNotFound',value,'Digital Read'));
                    end
                end
            end
    ...
    methods (Static)
        function name = getDescriptiveName()
            name = 'NXP FRDM-K64F Board Digital Read';
        end
    ...
    methods(Static, Access=protected)
        function header = getHeaderImpl()
            header = matlab.system.display.Header(mfilename('class'),...
                ...
                'Title','NXP FRDM-K64F Board Digital Read', ...
