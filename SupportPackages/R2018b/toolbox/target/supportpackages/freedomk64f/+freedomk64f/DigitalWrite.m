classdef (StrictDefaults)DigitalWrite < matlabshared.svd.DigitalWrite ...
            ...
            if ~coder.target('Rtw') && ~coder.target('Sfun')
                if ~isempty(obj.Hw)
                    if ~isValidDigitalPin(obj.Hw,value)
                        error(message('svd:svd:PinNotFound',value,'Digital Write'));
                    end
                end
            end
    ...
    methods (Static)
        function name = getDescriptiveName()
            name = 'NXP FRDM-K64F Board Digital Write';
        end
    ...
    methods(Static, Access=protected)
        function header = getHeaderImpl()
            header = matlab.system.display.Header(mfilename('class'),...
                ...
                'Title','NXP FRDM-K64F Board Digital Write', ...
                'Text', ['Set the logical state of a digital output Pin.' newline newline ...
                'The block input accepts logical and double data type and internally converts it to boolean.' newline newline ...
