function analogInputCallback(varargin)
%ANALOGINPUTCALLBACK Initializes the Analog Input block mask
...
switch action
    case 'MaskInitialization'
        set_param(blk, 'MaskDescription', DAStudio.message('freedomk64f:blocks:AnalogInputBlockDescription'));
        set_param(blk, 'MaskHelp','eval(''codertarget.internal.helpView(''''freedomk64f'''' ,''''frdmk64flibanaloginput'''')'');');
        set_param(blk, 'InitFcn', 'codertarget.freedomk64f.internal.analogInputCallback(''Validate'');')
    ...
    case 'Validate'
        validateAnalogInputblock(blk);        
    otherwise
        error('Unknown callback action "%s"', action)
end
end

function validateAnalogInputblock(current_block, varargin)
