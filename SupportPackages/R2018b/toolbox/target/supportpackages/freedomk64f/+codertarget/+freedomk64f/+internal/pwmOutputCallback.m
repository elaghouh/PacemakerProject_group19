function pwmOutputCallback(varargin)
%PWMOUTPUTCALLBACK Initializes the PWM Output block mask
...
switch action
    case 'MaskInitialization'
        set_param(blk, 'MaskDescription', DAStudio.message('freedomk64f:blocks:PWMOutputBlockDescription'));
        set_param(blk, 'MaskHelp','eval(''codertarget.internal.helpView(''''freedomk64f'''' ,''''frdmk64flibpwmoutput'''')'');');
        set_param(blk, 'InitFcn', 'codertarget.freedomk64f.internal.pwmOutputCallback(''Validate'');')
...
function validatePWMOutputblock(current_block, varargin)
